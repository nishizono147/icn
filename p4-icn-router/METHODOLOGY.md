...
| `chunk_table/benchmark_icn.py` | chunk ICN 取得時間計測 |
| `chunk_table/send_content.py` | chunk ICN Producer（AF_PACKET + 事前組立） |
| `ip_baseline/benchmark_ip_chunk.py` | chunk IP 取得時間計測 |
| `ip_baseline/serve_content_chunk.py` | chunk IP Producer（kernel UDP + 事前組立） |
| `run_compare_chunk_graph.py` | chunk 比較の一括実行・グラフ生成 |
| `results/compare_chunk_ip.csv` | chunk 比較生データ |
| `results/compare_chunk_ip.png` | chunk 比較グラフ |

---

## 11. chunk_table vs IP baseline（主実験）

pit_table（256 B 単一応答）に加え、**同一コンテンツを 256 B × 4 chunk で分割**した
`chunk_table` と IP baseline の比較を行う。発表の主張はこちらを用いる。

### 11.1 共通条件

| 項目 | 内容 |
|------|------|
| トポロジ | `chunk_table/topology.json`（3 スイッチ、h1–h2–h3） |
| コンテンツ | `content_id=4` / `image4.png`（1024 B = 4 chunks） |
| chunk サイズ | 256 B |
| 試行数 | 10（Mininet 12 セッションのうち先頭 2 を捨て） |
| 計測 | h1 の tcpdump pcap。Interest/UDP 要求 → **最終 chunk 到着**まで |
| Producer | 起動時に chunk をメモリキャッシュ。送信は事前組立済みバイト列 |

### 11.2 公平性（Producer 統一）

両 Producer は **同じ負荷モデル** とする。

- **ICN** (`chunk_table/send_content.py`): AF_PACKET、`prebuilt_cache`、Interest 受信ごとに 1 chunk 送信
- **IP** (`ip_baseline/serve_content_chunk.py`): kernel UDP、`prebuilt_payloads`、`chunk_id` 付き応答

転送経路の差（ICN: L2 カスタム / IP: L3 UDP）は **ICN の設計上の利点** として許容する。
Producer 側の Scapy 逐次組立などの非対称は排除済み。

### 11.3 比較の見方（Cold / Warm ラベルなし）

試行 1〜10 を **そのまま対応** させ、各試行で ICN と IP の取得時間を並べる。
「Cold」「Warm」という区別は設けない。

- グラフ: 横軸 = 試行番号、縦軸 = 取得時間（ms）、ICN / IP の 2 曲線
- 主張: 各試行（または 10 試行平均）で **ICN < IP** なら「ICN の方が速い」

中継キャッシュ（trial 2 以降で ICN が短くなる現象）は、PIT/chunk テーブルの
**設計上の効果** として説明する。IP 側に同等機構はない。
...
sudo env PATH=/home/p4/src/p4dev-python-venv/bin:$PATH \
  ...
  python3 run_compare_chunk_graph.py -s 12 --report-sessions 10
```

### 11.5 将来設計との関係

`設計メモ.txt` にある **IP 上に ICN を載せる過渡期設計**（L3 転送 + 中継キャッシュ）は、
本実験（L2 ICN vs 素の IP baseline）とは別フェーズである。
本節の比較は **L2 ICN ルータ実装** と **IP baseline** の性能差を示すためのもの。
